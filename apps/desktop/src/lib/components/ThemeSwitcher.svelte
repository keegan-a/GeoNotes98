<script lang="ts">
  ...
  let statusMessage = '';
  let lastAppliedTheme: ThemeId | null = null;

  function applyTheme(theme: ThemeId) {
    if (!isClient) return;
    if (!rootEl) return;
    // Remove previous theme classes to prevent stacking conflicting variables.
    rootEl.classList.remove(...THEME_CLASSES);
    if (!rootEl.classList.contains(theme)) {
  ...
  $: if (initialized && isClient && selectedTheme !== lastAppliedTheme) {
    applyTheme(selectedTheme);
